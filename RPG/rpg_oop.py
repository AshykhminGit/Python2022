...
def meetShop (): # функция магазина это потом можно будет разнести по файлам и классам
    ...
    weaponLvl = r.randint(1,5) # рандомный уровень оружия вторая цифра в данном случае 5 напрямую завист от количества слов в weaponrarities
...
# список оружия создаем
    ...
    weaponRarities = ["Испорченный", "Редкий", "Легендарный", "Устрашающий", "Ржавый"]
    ...
    while input("Ты смотришь на магазин. Что ты будешь делать дальше? зайти? уйти?").lower() == "зайти":
        print(f" 1) на прилавке простое зелье здоровья восстанавливает 1 еденицу здоровья -  стоит {oneHpCost} монет ")
        print(f" 2)  на прилавке тройное зелье здоровья  восстанавливает 3 единицы здлоровтя и стоит {threeHpCost} монет ")
        print(" 3) на прилавке {0} {1} - {2} монет,  его урон {3} ".format(weaponRarity, weapon, weaponCost, weaponDmg))
...
#  ГЭЙМ ЛУП  В КОТОРОМ ИДЕТ ВЫБОР КТО НАМ ВСТРЕТИТСЯ
...
InitGame(5, 10, 5)  #тут можно ввести начальные параметры при иницализации игры
printHp()
...
while True:
    ...
    if hp <= 0:
        if input("Хочешь начать сначала ? Да 1 ?Нет 2 ").lower == "1":
            InitGame(5, 10, 5)
        else:
            break
